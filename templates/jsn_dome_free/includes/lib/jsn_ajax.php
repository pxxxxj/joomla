...
class JSNAjax
{
	...
	function downloadSampleDataPackage()
	{
		...
		$foldername 	= 'tmp';
		$folderpath 	= JPATH_ROOT . DIRECTORY_SEPARATOR . $foldername;
		$link 			= '';
		...
		if (is_writable($folderpath))
		{
			...
			if ($result)
			{
				...
				else
				{
					...
					if (JFile::exists(JPATH_ROOT . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR . $result))
					{
								...
								foreach ($installExts as $ext)
								{
									...
									if ($ext->author === 'joomlashine')
									{
    									if ($ext->show === true)
    									{
    										...
    										if ($checkResult->toInstall === true ||
    											(isset($extList[$ext->name]['deps']) && count($extList[$ext->name]['deps'])) )
    										{
		...
		if ($unpackage)
		{
			...
			if ($installed_data && is_array($installed_data))
			{
				if (trim($sample_xml_data['version']) != trim($this->_template_version))
				{
					...
					echo json_encode(array('install' => false, 'message' => JText::sprintf('JSN_SAMPLE_DATA_OUTDATED_PRODUCT', $this->_template_version), 'redirect_link'=>$link));
					return;
				}
				else
				{
