...
# @version $Id: jsn_sampledata_helper.php 17424 2012-10-25 04:02:22Z tuyetvt $
-------------------------------------------------------------------------*/
...
class JSNSampleDataHelper
{
		...
		if ($unpack_package)
		{
			...
			if ($installed_data && is_array($installed_data))
			{
				if (trim($sample_data_xml['version']) != trim($template_manifest['version']))
				{
					...
					$msg = JText::sprintf('JSN_SAMPLE_DATA_OUTDATED_PRODUCT', $template_manifest['version'])
						. '&nbsp;<a class="link-action" href="index.php?template=' . strtolower($this->_template_name) . '&tmpl=jsn_autoupdater">'
						. JText::sprintf('JSN_SAMPLE_DATA_UPDATE_LINK_TEXT', $sample_data_xml['version']) . '</a>';
	...
	function determineFailedExtensions(&$failedExts, $extInfoArray, $selectedExts = array())
	{
		...
		foreach ($notSelected as $ext)
		{
			...
			elseif ($extInfoArray[$ext]->type === 'component')
			{
				$failedExts[$ext]['exist']   = $extInfoArray[$ext]->exist;
			}
		}
	...
	/**
	 * Set Articles and Categories's author to Super User as the super user's id created
	 * by Joomla 3.0 may vary. In Joomla 2.5, it's fixed to 42.
	 */
	public function setArticleAuthor()
	{
		$db = JFactory::getDBO();

		/* Determine root user's id */
		$query = 'SELECT id FROM #__users WHERE LOWER(name) = "super user"';
		$db->setQuery($query);
		$superuserId = $db->loadResult();

		if ($superuserId)
		{
			$query = 'UPDATE #__content SET created_by = ' . $superuserId . ', '
				. ' modified_by = ' . $superuserId;
			$db->setQuery($query);
			$db->query();

			$query = 'UPDATE #__categories SET created_user_id = ' . $superuserId . ', '
				. ' modified_user_id = ' . $superuserId
				. ' WHERE created_user_id = 42';
			$db->setQuery($query);
			$db->query();
		}
	}
